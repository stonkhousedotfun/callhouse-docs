...
| Close | Burn equal long and short units before settlement to free their collateral. Deployed v7 returns unused rent to the closer only when the close is before expiry. |
...
| Mint rent | Deployed v7 fee charged in the collateral asset whenever new option units are minted, based on locked collateral and time to expiry. Unused rent can be refunded on a pre-expiry close. |
...
| Stale auto-roll ask | A tracked auto-roll ask whose strike has been reached by a fresh oracle spot. Deployed v7 lets anyone cancel its unfilled remainder, but a buyer can fill first. |
...
| Vault outflow cap | Deployed v7 refillable limit on net USDG the treasury maker vault pays through quoter calls. It does not limit all trading or settlement losses. |
