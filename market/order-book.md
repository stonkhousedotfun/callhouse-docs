...
| Write-on-fill ask | Nothing is escrowed and nothing is minted yet | The maker's free balance pays the locked collateral in the same asset; the book mints a new long/short pair and the buyer receives the long. |

**Why a quote can vanish.** A maker can cancel, an order can fill, expire, or pass the writing cutoff. A write-on-fill maker can use its free collateral elsewhere, leaving too little for an earlier ask's collateral. The indexer should hide such asks when it sees the shortfall, but the chain rechecks at execution. An auto-roll ask may also be cancelled after a fresh oracle spot reaches its strike; that requires a transaction and can arrive after a fill. The app re-reads chosen order ids and simulates the transaction before sending. A reverted transaction can still cost gas.
...
A book-fee change is announced 48 hours before it takes effect, and the role that schedules it waits 48 hours of its own before it can even be sent, so a change is visible well before it bites. A replacement schedule restarts the on-chain delay; scheduling the current fees cancels a pending change. An order that remains open uses the fees effective when it fills, including a change announced after placement. Makers can cancel or replace during the notice. A taker's price limit does not limit its fee; `TakeParams.maxTotalFee` does. The book checks it on every take: if the taker-side fees, the taker fee plus your own seller fees when you sell into bids, would exceed it, the take reverts with `FeeAboveMax` instead of paying the new rate. A value of 0 permits only a take that owes no fee, and `type(uint128).max` removes the cap, so set it from the `takerFee` and `sellerFees` that `quoteTake` returns, as the app does.
