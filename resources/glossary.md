...
**Admin Safe**
A 2-of-3 Safe multisig that holds the vault's admin role. It sets policy inside the hard caps, the deposit cap, the price-age limit and the fee recipient, decides whether to accept Valorem's engine fee, and lifts a halt. It cannot move depositors' tokens. See [Roles and admin powers](../protocol/roles.md).

**Assignment**
What happens to the writer of a call when a holder exercises it. Valorem takes the vault's NVDA collateral at the strike and leaves the strike price in USDG. Assignment can take the collateral at the strike. It can be partial: some of the vault's contracts assigned and the rest not. See [Assignment](../product/assignment.md).
...
**Guardian**
A single-key role that can halt new writes and cancel listings, and nothing else. It cannot touch collateral, change policy or stop a withdrawal. Lifting a halt needs the Admin Safe.
...
**Registry**
Overcall's per-market contract that defines each weekly cycle: its number, the approved strike rungs, the exercise timestamp and the expiry. The vault binds to it instead of a calendar.
...
**Safe**
A smart-contract multisig wallet that needs a set number of signers to act. Callhouse's admin role is held by a 2-of-3 Safe, and the protocol fee is paid to a fee Safe.

**Share price**
NVDA per cNVDA share: the vault's idle NVDA plus NVDA locked in the current call, divided by the share supply. It moves only when the vault's NVDA balance moves.
