...
| `Clearinghouse.registerMarket`, `setMarketConfig`, `setCalendar` | Enable or disable markets; change the tick, oracle, exercise fee, proposed rent rate or calendar for new series. A new oracle can misprice or strand new collateral. | 18-decimal underlying; tick a non-zero multiple of 100; exercise fee at most 200 bps; oracle and calendar must have code. Proposed rent ceiling is 5,000 parts per million of collateral per seven days remaining. Existing series keep their oracle, expiry and pinned fees. |
...
| `AutoRoller` bounty and minimum-roll setters; grant `PRICER_ROLE` | Change automation economics and who may reprice smart-pricing asks. | Proposed v7 `reprice` also refuses an in-the-money tracked ask; price remains inside each writer's chosen band and size and expiry stay fixed. |
| Payout adapter route; maker vault limits and treasury controls; maker rebate tiers; weekly reward roots | Change conversion pool and treasury market-making or reward allocation. | Clearinghouse conversion floor, vault quote/exposure guards and proposed refillable net USDG outflow cap, rebate bound and one root per reward epoch. The vault's funds are treasury funds. The admin can change the outflow limit and is exempt from it. |
...
| `PRICER_ROLE` | `AutoRoller.reprice` | Replaces only a smart-pricing writer's ask, inside that writer's own price band; proposed v7 refuses it when spot is at or past the strike. |
| `QUOTER_ROLE` | `MakerVault` quote and trade calls | Trades treasury inventory within per-series, total exposure, bid/ask, live-order and proposed net USDG outflow guards. It cannot name an outside withdrawal recipient. |
| No role | Create series; snapshot, finalise, settle, prune, redeem, roll, proposed `AutoRoller.cancelStale` and sweep fees | Permissionless lifecycle calls. The caller cannot redirect a holder's collateral or payout. `cancelStale` needs a fresh oracle spot and cancels only the unfilled tracked ask. |
...
The interface v7 source contains a compiled 30-minute opening grace for auto-roll, a seven-day rent denominator and a 24-hour vault outflow refill window. Recheck the final reviewed revision, tests and production-deployed values before publication.
