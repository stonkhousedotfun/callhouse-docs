...
**What the system does:** closes the week flat and publishes it alongside the filled ones, with premium 0. The listing is not posted to any marketplace or order-book service. The keeper serves it to the app's cycle page, and it is validated on chain, where any Seaport 1.6 client can fill it, but buyers have to find it. If the keeper is down, the cycle page has no order to show, and only someone who already has the order's details can fill it.
...
**What the system does:** the keeper's ask is at least 0.5% above the premium floor, and its strike is at least 5% above spot when the week is armed, 2 percentage points above the band floor, so a small rise does not immediately stop sales. Each tick it repeats the vault's spot checks: it reprices when the price is the problem and a listing is left, and records an alert when it cannot. For cycle 1's listing (strike 223 USDG, 0.856436 USDG per contract) under the current policy, the band floor refuses fills above a spot of about 216.50 USDG, about 2.2% above the 211.92 USDG the feed showed on 15 September 2026; at 0.10% the premium floor would not refuse that price below about 856 USDG. Those thresholds are computed, not observed. In the keeper's extended fork rehearsal, run with the earlier 0.40% floor, a 1.5% rise refused a fill; the keeper relisted at the new floor plus its 1% margin with the week's third listing, and the next fill went through. A fourth listing that week was refused.

### Assignment caps your upside

Anyone holding the calls of that week's option type can exercise them during the exercise window. Valorem assigns an exercise pro rata by amount written across the writers in a bucket, and every fill of the vault's week lands in the same bucket as anything other writers wrote before the first exercise. So the vault can be assigned because of calls other people sold or wrote and exercised themselves. It can never be assigned on more contracts than it sold, because it writes only what buyers take. For each assigned contract, Valorem takes NVDA at the strike and the vault receives strike USDG, credited to depositors without a fee.
...
Instant redemption is off from the moment a week is armed until it closes, even in a week with nothing sold: `redeem` and `withdraw` revert with `UseQueue` unless the vault is Idle with nothing written (`canRedeemInstantly()`). A withdrawal started mid-week settles when the week closes. A redemption queued while the vault is Idle can be settled straight away by anyone with `settleQueue()`. A queued redemption cannot be cancelled. cNVDA is not listed anywhere, so there is no secondary market to exit into. See [Withdrawing and the redeem queue](../getting-started/withdrawing.md).
...
* **A wrong price that looks fresh.** The vault rejects only a zero or negative answer. Chainlink warns that tokenised-equity feeds can surface atypical values, zero included, with fresh timestamps, particularly overnight. A wrong price with a fresh timestamp passes every check the vault makes. The keeper uses Cboe's quotes only when the feed's price is within 3% of the NVDA share price in them: when it arms a week it skips the week otherwise, and for a later listing it falls back to the fair value stored earlier in the week or lists nothing. The vault's own listing and fill checks read the feed alone. The feed's own administrators, a 4-of-9 Safe, can also replace its data source.
...
* An internal review of the redesigned contracts on 2026-09-14 found no Critical, High or Medium issue. It found one Low: a buyer that is a contract could deposit inside its own fill and take part of that fill's premium. Deposits are now refused in any transaction in which a fill has written. It also raised one Informational item, about the engine-fee switch on Stonkhouse's own clearinghouse (see [Fees](fees.md#who-holds-the-valorem-fee-switch)).
...
**Source verification.** The vault's source, and the source of its two libraries, is verified on Sourcify as a partial match (Sourcify's `match`: the code matches, the metadata hash does not). Stonkhouse's clearinghouse at `0x53d7A6d0489Daf3d67b9A314e0eAB2B78Acab9C6` is not source-verified on Sourcify or Blockscout. Its runtime bytecode is identical, except for the metadata hash, to the Valorem clearinghouse at `0x9a7b40e5c1dB1Af822ef091c990b58b02C78C0C0`, whose source Sourcify verifies as an exact match and which is Valorem's upstream code at commit `6436c823`.

**Costs you:** in the worst case, everything deposited. The deposit cap, 20 NVDA today (`depositCap()`), is the honest measure of how much confidence this deserves. Details: [Security and audits](../protocol/security.md).

### Keeper failure

The keeper is one hot key running a weekly loop. It can crash, run out of gas, or miss the window.

**Costs you:** a skipped week if it stops before arming. If it stops while a week is Listed, the cycle page has no order to show, so the week probably sells nothing more; nothing is written without a fill, so a stopped keeper locks up no extra collateral. If it stops after fills, only delay.
...
Nobody is paged when the keeper fails. It logs its alerts and stores them in its database, but no webhook or relay delivers them to anyone today.
...
**Costs you:** a compromised keeper can arm the lowest strike the band allows, list the vault's whole capacity at exactly the premium floor, and have a buyer it controls fill it at once. Nothing requires a delay between a listing and its first fill, so a guardian cannot react in time. The project's security model values a 7-day call 3% out of the money at about 1.55% of spot at 50% implied volatility. Under the current policy (strike floor 3%, premium floor 0.10%), that puts the buyer's expected gain, which is depositors' expected loss, at about **1.45% of the notional sold per week** at 50% implied volatility and about 3.0% at 80%, with up to 95% of the vault sold. At the 0.40% floor the vault launched with, the same arithmetic gave about 1.15% and 2.7%. Nothing on chain notices a sale at the floor, so it repeats every week until someone halts. A compromised keeper still cannot take a token out of the vault, sell above the strike, list past the exercise timestamp, or write outside the band and the caps, and with calls written only when bought there is no unsold inventory to exploit.

**The honest keeper can underprice too.** It asks at least 10% over the mid of Cboe's delayed NVDA quotes, but those quotes are delayed, a weekend arm prices on the last session's close, and the keeper reprices a live listing upward only when the market-based ask is more than 25% above it. Under the ask sits only the premium floor plus 0.5%. Cycle 1's listing was priced before this mode, from the floor plus a margin. Depositors bear any gap between the ask and what the call is worth without anyone being compromised.

Mitigations considered and **not** implemented: a timelock on admin actions, higher compiled floors, a delay before a new listing can be filled, and no deposits before the admin role moves to a Safe. None of these is in the code. Pricing from option quotes, also on that list, is now the keeper's `vol` mode; it helps against an honest keeper's mispricing and does nothing against a compromised key. See [Roles and admin powers](../protocol/roles.md).

### Admin judgment and admin key compromise

The vault admin sets policy inside the compiled caps: the strike band, the premium floor, the utilisation limit, the protocol fee, the contract cap, the deposit cap, the price-age limit, the fee recipient, and whether to accept Valorem's engine fee. It halts and unhalts, and grants and revokes every role, the keeper's included. The admin role is held by one hot key, `0xEb82c3D0F89d47453F94f0C2b2a2752e27a19d9b`, which deployed the vault and is also its fee recipient. Moving the role to a Safe is planned and has not happened. The clearinghouse's fee switch is held separately, by a 1-of-1 Safe. There is no timelock on any of it.
...
Valorem Clear holds the collateral, mints the calls and settles assignment. Its upstream developer has shown no public activity since January 2024, and the last commit to the clearinghouse contract is from August 2023 (the last to its repository, from November 2023). It has published no security advisories, and there is no bug bounty, no incident response and no patch path. Zellic reviewed the clearinghouse twice, in December 2022 and January 2023, and reviewed a patch in August 2023. Two of its findings about how exercises pick buckets (the choice is not weighted by size, and the next bucket is known in advance) were acknowledged and never fixed. Those reviews cover Valorem, not this vault.
...
The vault uses Stonkhouse's own instance of the Valorem clearinghouse, `0x53d7A6d0489Daf3d67b9A314e0eAB2B78Acab9C6`, rather than an instance someone else operates. Its runtime bytecode matches Valorem's upstream code at commit `6436c823` except for the metadata hash, and its source is not verified on Sourcify or Blockscout (see [Smart contract risk](#smart-contract-risk)). The clearinghouse has no owner, no pause, no blocklist and no proxy. Its one privileged role, the fee holder (`feeTo()`), is the Safe `0xff1454009F024507f3E455eb2027E98fAF4ccF61`, with one owner and a threshold of 1. That Safe can switch the 15 bps engine fee on, sweep the fees collected, change the metadata renderer and nominate a successor, with no timelock, and a nomination emits no event until it is accepted. It cannot touch collateral.

**Costs you:** the engine-fee lever described on [Fees](fees.md#who-holds-the-valorem-fee-switch). Switched on without the vault admin's acceptance, it stops every arm and fill; switched on and accepted, it takes NVDA from the vault on every fill.
...
| **Chainlink RHNVDA/USD feed** | Gates arming, listings and fills | No prints while the market is closed; a wrong price with a fresh timestamp passes every check the vault makes; a 4-of-9 Safe controls its data source. |
| **Valorem Clear** | Holds the collateral, mints the calls, settles assignment | Unmaintained upstream. On Stonkhouse's own instance, a 1-of-1 Safe holds the engine-fee switch, and its source is not verified on Sourcify or Blockscout. |
...
Each document is stamped with its version. **No operating entity and no governing law have been designated yet**, and the pages say so in words rather than naming a placeholder. The contracts themselves are on a public chain and are not governed by the Terms.
